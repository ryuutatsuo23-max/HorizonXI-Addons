...
Version 0.8.0 is not a complete HorizonXI checklist. It includes four bounded pieces:
...
The `Maps` tab has its own compact selector for `All Maps`, `Original Areas`, `Rise of the Zilart`, `Chains of Promathia`, and `Treasures of Aht Urhgan`. Its Map, Source, and vendor Price columns are aligned and separated by visible draggable dividers. Thirty maps have a sourced vendor price; the other 42 show a neutral dash rather than a guessed zero cost. The list is intentionally limited to the 72 rows in the sourced HorizonXI table; additional retail client map records and map pages outside that table are not silently imported.
