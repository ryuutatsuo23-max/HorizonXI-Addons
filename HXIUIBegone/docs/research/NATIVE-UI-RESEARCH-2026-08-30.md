...
| --- | --- | --- |
| Target box versus overhead arrows | `targetwi` constructor points to vtable RVA `0x332DA8`; draw callback is RVA `0x14EBA0`. Its later sprite blocks at `0x14EF1C` and `0x14EF5A` use the SDK-documented arrow coordinates at `0xBC` through `0xC2`; earlier box drawing uses the distinct box/icon coordinates at `0x4C` through `0x52`. | Version 0.2.5 moved only the four box/icon coordinates off-screen and left the primitive plus arrow coordinates untouched. The author confirmed the box hides while the selected-target arrow remains visible in normal play. |
...
| Cast bar | Native `casttime` record, object-slot RVA `0x575F0C`. The full 44-byte descriptor is unique in the supported offline build. | Added as a guarded control in 0.2.4 using the existing primitive ownership path. The author confirmed it hides the bar in game, with a possible sub-second initial flash. |
